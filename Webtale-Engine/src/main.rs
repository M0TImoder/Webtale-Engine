...
const Z_BORDER: f32 = 5.0;
const Z_BG: f32 = 6.0;

const Z_ATTACK_TARGET: f32 = 10.0;
const Z_ATTACK_BAR: f32 = 11.0;

const Z_SLICE: f32 = 15.0;

const Z_BUTTON: f32 = 20.0;
const Z_HP_BAR_BG: f32 = 20.0;
const Z_HP_BAR_FG: f32 = 21.0;
const Z_TEXT: f32 = 22.0;

const Z_DAMAGE_HP_BAR: f32 = 25.0; 
const Z_DAMAGE_TEXT: f32 = 26.0;   

const Z_BUBBLE: f32 = 30.0; 
const Z_BUBBLE_TEXT: f32 = 31.0;

const Z_SOUL: f32 = 40.0;

const MENU_LAYER_TOP: i32 = 0;
const MENU_LAYER_FIGHT_TARGET: i32 = 1;
const MENU_LAYER_ACT_TARGET: i32 = 2;
const MENU_LAYER_ITEM: i32 = 3;
const MENU_LAYER_MERCY: i32 = 4;
const MENU_LAYER_ACT_COMMAND: i32 = 10;
...
#[derive(Component)] struct HpBarRed;
#[derive(Component)] struct HpBarYellow;
...
#[derive(Component)] struct EnemyBody; 
#[derive(Component)] struct EnemyHead { base_y: f32, timer: f32 }
#[derive(Component)] struct ActCommands { commands: Vec<String> }
#[derive(Component)] struct MenuTextItem { layer: i32, index: i32 }
#[derive(Component)] struct MainDialogText;
#[derive(Component)] struct Vaporizing { scan_line: f32, image_handle: Handle<Image>, initial_y: f32 }
#[derive(Component)] struct DustParticle { velocity: Vec3, timer: Timer, max_alpha: f32 }
#[derive(Component)] struct SpeechBubble;
#[derive(Component)] struct AttackTargetBox;
#[derive(Component)] struct AttackBar { speed: f32, moving: bool, flash_timer: Timer, flash_state: bool }
#[derive(Component)] struct SliceEffect { timer: Timer, frame_index: usize }
#[derive(Component)] struct PendingDamage { timer: Timer, damage: i32, target_pos: Vec3 }
#[derive(Component)] struct DamageNumber { timer: Timer, velocity_y: f32, gravity: f32, start_y: f32 }
#[derive(Component)] struct EnemyHpBar { lifespan: Timer, animation: Timer, start_width: f32, target_width: f32 }
#[derive(Component)] struct EnemyHpBarForeground;
#[derive(Component)] struct BorderVisual;
#[derive(Component)] struct BackgroundVisual;
...
fn main() {
    App::new()
        ...
        .add_systems(Update, (
            handle_global_input,
            camera_scaling_system,
            menu_input_system,
            menu_render_system,
            soul_position_sync,
            soul_combat_movement,
            update_box_size,
            draw_battle_box,
            draw_ui_status,
...
fn spawn_game_objects(commands: &mut Commands, asset_server: &AssetServer, game_fonts: &GameFonts) {
...
fn update_box_size(mut box_res: ResMut<BattleBox>, time: Res<Time>, game_state: Res<GameState>) {
...
fn battle_flow_control(
    ...
    if game_state.mnfight == 1 {
        ...
        if is_finished && input.just_pressed(KeyCode::KeyZ) {
            ...
            game_state.bubble_timer = Timer::from_seconds(5.0, TimerMode::Once); 
